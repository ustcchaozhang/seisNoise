...
if [ $system == 'slurm' ]; then
    ...
    sh pbsssh.sh  $SCRIPTS_DIR/prepare_data_pbs.sh $velocity_dir $SUBMIT_DIR $SCRIPTS_DIR $SUBMIT_RESULT $WORKING_DIR 2> ./job_info/error_target
else
    echo "no working system can be used"
    temp_prepare="./job_info/output_prepare"
    sh  $SCRIPTS_DIR/prepare_data.sh $velocity_dir $SUBMIT_DIR $SCRIPTS_DIR $SUBMIT_RESULT $WORKING_DIR >$temp_prepare 2> ./job_info/error_target
fi
...
do
    ...
    if [ $system == 'slurm' ]; then
        ...
        echo "sh pbsssh.sh  $SCRIPTS_DIR/Adjoint_pbs.sh $velocity_dir $compute_adjoint"
        sh pbsssh.sh  $SCRIPTS_DIR/Adjoint_pbs.sh $velocity_dir $compute_adjoint $SUBMIT_DIR $SCRIPTS_DIR $SUBMIT_RESULT $WORKING_DIR 2> ./job_info/error_current
    else
        temp_adjoint="./job_info/output_adjoint_"${iter}
        sh $SCRIPTS_DIR/Adjoint.sh $velocity_dir $compute_adjoint $SUBMIT_DIR $SCRIPTS_DIR $SUBMIT_RESULT $WORKING_DIR >$temp_adjoint 2> ./job_info/error_current
    fi

   # if [ $iter -eq $iter_start   ]; then
   #     echo 
   #     echo "copy the working fold for $iter"
   #     cp -r $working_path/$Job_title  $working_path/$iter
   #  fi

   # if [ $iter -eq $iter_end   ]; then
   #     echo 
   #     echo "copy the working fold for $iter"
   #     cp -r $working_path/$Job_title  $working_path/$iter
   # fi
    ...
    temp_datamisfit="./job_info/output_datamisfit_"${iter}
    ./bin/data_misfit.exe $iter $step_length $compute_adjoint $NPROC_SPECFEM $WORKING_DIR $SUBMIT_RESULT >$temp_datamisfit 2> ./job_info/error_data_misfit_$iter
    ...
    if $smooth ; then
        ...
        echo "./bin/xsmooth_sem $smooth_x $smooth_z $z_precond $kernel_list,$precond_list  $SUBMIT_RESULT/misfit_kernel/ $SUBMIT_RESULT/misfit_kernel/ $GPU_MODE"
    ...
    echo "./bin/optimization.exe $NPROC_SPECFEM  $SUBMIT_RESULT $kernel_list $precond_list  $model_list $iter"
    ...
    if [ $iter -eq $iter_start   ]; then
        echo 
        echo "copy the working fold for $iter"
        cp -r $SUBMIT_RESULT/misfit_kernel/ kernel_01
    fi  
    ...
    do
        ...
        if [ $system == 'slurm' ]; then
            ...
            sh pbsssh.sh  $SCRIPTS_DIR/Adjoint_pbs.sh $velocity_dir $compute_adjoint $SUBMIT_DIR $SCRIPTS_DIR $SUBMIT_RESULT $WORKING_DIR 2> ./job_info/error_update_$iter
        else
            sh $SCRIPTS_DIR/Adjoint.sh $velocity_dir $compute_adjoint $SUBMIT_DIR $SCRIPTS_DIR $SUBMIT_RESULT $WORKING_DIR 2> ./job_info/error_update_$iter
        fi
