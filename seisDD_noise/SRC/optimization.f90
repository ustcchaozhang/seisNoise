...
subroutine initialize(directory,kernel_names,precond_name,model_names)
    ...
    allocate(m_new_rho(NGLLX*NGLLY*NGLLZ*NSPEC*nmod))
    m_new_rho = 0.0_CUSTOM_REAL
   ...
   ! enddo  ! myrank
    ...
    enddo ! imod
    enddo ! myrank
    m_new_rho=reshape(temp_store_model,shape(m_new_rho))
   ...
   !! by chao, here we use the empire function for vp and rho 
    ...
    if(empire_function) then
            ...
            0.0251*4*(m_new(NGLLX*NGLLY*NGLLZ*NSPEC*1/2)/1000)**3)* ( m_new(NGLLX*NGLLY*NGLLZ*NSPEC*1/2) / m_new_vp(NGLLX*NGLLY*NGLLZ*NSPEC*1/2) )
            ...
            0.000106*5*(m_new_vp(NGLLX*NGLLY*NGLLZ*NSPEC*1/2)/1000)**4) * ( m_new_vp(NGLLX*NGLLY*NGLLZ*NSPEC*1/2) / m_new_rho(NGLLX*NGLLY*NGLLZ*NSPEC*1/2) ) 
