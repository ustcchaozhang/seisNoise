#!/bin/bash
module load intel/2018.2 openmpi/3.1.0
ulimit -s unlimited
# cd workdir
...
if [ $system == 'slurm' ]; then
    srun -n $ntasks -c $NPROC_SPECFEM -l -W 0 $SCRIPTS_DIR/Adjoint.sh $velocity_dir $compute_adjoint $SUBMIT_DIR $SCRIPTS_DIR $SUBMIT_RESULT $WORKING_DIR 2> ./job_info/error_current
elif [ $system == 'pbs' ]; then
    sh $SCRIPTS_DIR/pbsssh.sh  $SCRIPTS_DIR/Adjoint.sh $velocity_dir $compute_adjoint $SUBMIT_DIR $SCRIPTS_DIR $SUBMIT_RESULT $WORKING_DIR 2> ./job_info/error_current
fi
    ...
    echo "mpirun -np $NPROC_SPECFEM ./bin/sum_kernel.exe $kernel_list,$precond_list $WORKING_DIR $SUBMIT_RESULT 2> ./job_info/error_misfit_kernel_$iter"
    ...
    if $smooth ; then
        ...
        echo "mpirun -np $NPROC_SPECFEM ./bin/xsmooth_sem $smooth_x $smooth_z $z_precond $kernel_list,$precond_list  $SUBMIT_RESULT/misfit_kernel/ $SUBMIT_RESULT/misfit_kernel/ $GPU_MODE"
    ...
    do
        ...
        if [ $system == 'slurm' ]; then
            srun -n $ntasks -c $NPROC_SPECFEM -l -W 0 $SCRIPTS_DIR/Adjoint.sh $velocity_dir $compute_adjoint $SUBMIT_DIR $SCRIPTS_DIR $SUBMIT_RESULT $WORKING_DIR 2> ./job_info/error_update_$iter
        elif [ $system == 'pbs' ]; then
            echo "sh pbsssh.sh  $SCRIPTS_DIR/Adjoint.sh $velocity_dir $compute_adjoint $SUBMIT_DIR $SCRIPTS_DIR $SUBMIT_RESULT $WORKING_DIR 2"
            sh $SCRIPTS_DIR/pbsssh.sh  $SCRIPTS_DIR/Adjoint.sh $velocity_dir $compute_adjoint $SUBMIT_DIR $SCRIPTS_DIR $SUBMIT_RESULT $WORKING_DIR 2> ./job_info/error_update_$iter
        ...
        if [ $is_done -eq 1 ]; then
            ...
            cp $SUBMIT_RESULT/misfit_kernel/*kernel.bin   $SUBMIT_RESULT/m_$iter/
            break 
        fi
