...
sed -e "s#^MOD_DIR=.*#MOD_DIR=$package_path/lib/src#g"  $FILE > temp;  mv temp $FILE
...
if [ $system == 'slurm' ]; then
    ...
    qsub -q $queue -l nodes=$nodes:ppn=$max_nproc_per_node  -l walltime=$WallTime -e job_info/error -o job_info/output -V $Job_title.sh
