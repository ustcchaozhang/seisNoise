program data_misfit
    ...
    INTEGER :: i,j
    INTEGER :: ier
    real(kind=CUSTOM_REAL) ::misfit_cur,misfit_cur_mean,misfit_cur_1,misfit_cur_2,misfit_cur_3,misfit_cur_4,misfit_cur_5,misfit_cur_6,misfit_cur_7,misfit_cur_average
    real(kind=CUSTOM_REAL) ::misfit_cur_f1,misfit_cur_f2,misfit_cur_f3,misfit_cur_f4,misfit_cur_f5,misfit_cur_f6,misfit_cur_f7
    real(kind=CUSTOM_REAL) ::misfit_cur_mean1,misfit_cur_mean2,misfit_cur_mean3,misfit_cur_mean4,misfit_cur_mean5,misfit_cur_mean6,misfit_cur_mean7
    real(kind=CUSTOM_REAL) ::misfit_cur_std1,misfit_cur_std2,misfit_cur_std3,misfit_cur_std4,misfit_cur_std5,misfit_cur_std6,misfit_cur_std7
    character(len=MAX_STRING_LEN) :: arg(NARGS)
    character(len=MAX_STRING_LEN) :: input_dir,output_dir
    character(len=MAX_FILENAME_LEN) ::FILENAME,measurement_type
    ...
    call sum_misfit_mean(input_dir,misfit_cur,misfit_cur_f1,misfit_cur_f2,misfit_cur_f3,misfit_cur_f4,misfit_cur_f5,misfit_cur_f6,misfit_cur_f7,misfit_cur_mean1,misfit_cur_mean2,misfit_cur_mean3,misfit_cur_mean4,misfit_cur_mean5,misfit_cur_mean6,misfit_cur_mean7,misfit_cur_std1,misfit_cur_std2,misfit_cur_std3,misfit_cur_std4,misfit_cur_std5,misfit_cur_std6,misfit_cur_std7,NPROC_DATA)
    if (measurement_list == 'WD') then 
        call sum_misfit_all(input_dir,misfit_cur_1,misfit_cur_2,misfit_cur_3,misfit_cur_4,misfit_cur_5,misfit_cur_6,misfit_cur_7,NPROC_DATA)
    endif
    ...
    if(iter>0) then ! doing inversion/kernel
        ...
        OPEN (IOUT, FILE=trim(filename),status='unknown',POSITION='APPEND')
        write(IOUT,'(i,f15.5,8f15.5)') iter,step_length,misfit_cur_f1,misfit_cur_f2,misfit_cur_f3,misfit_cur_f4,misfit_cur_f5,misfit_cur_f6,misfit_cur_f7,misfit_cur
        close(IOUT)


        write(filename, "(a)") trim(output_dir)//'/misfit/mean_data_misfit_hist_detail'
        OPEN (IOUT, FILE=trim(filename),status='unknown',POSITION='APPEND')
        write(IOUT,'(i,f15.5,7f15.5)') iter,step_length,misfit_cur_mean1,misfit_cur_mean2,misfit_cur_mean3,misfit_cur_mean4,misfit_cur_mean5,misfit_cur_mean6,misfit_cur_mean7
        close(IOUT)

        write(filename, "(a)") trim(output_dir)//'/misfit/std_data_misfit_hist_detail'
        OPEN (IOUT, FILE=trim(filename),status='unknown',POSITION='APPEND')
        write(IOUT,'(i,f15.5,7f15.5)') iter,step_length,misfit_cur_std1,misfit_cur_std2,misfit_cur_std3,misfit_cur_std4,misfit_cur_std5,misfit_cur_std6,misfit_cur_std7
        close(IOUT)
...
subroutine sum_misfit_mean(directory,misfit_cur,misfit_cur_f1,misfit_cur_f2,misfit_cur_f3,misfit_cur_f4,misfit_cur_f5,misfit_cur_f6,misfit_cur_f7,misfit_cur_mean1,misfit_cur_mean2,misfit_cur_mean3,misfit_cur_mean4,misfit_cur_mean5,misfit_cur_mean6,misfit_cur_mean7,misfit_cur_std1,misfit_cur_std2,misfit_cur_std3,misfit_cur_std4,misfit_cur_std5,misfit_cur_std6,misfit_cur_std7,NPROC_DATA)
    use seismo_parameters
    implicit none
    integer :: irec,ier,isrc,ip,NPROC_DATA,inum,num,num_mean
    real(kind=CUSTOM_REAL) ::misfit_cur_std,misfit_cur_std1,misfit_cur_std2,misfit_cur_std3,misfit_cur_std4,misfit_cur_std5,misfit_cur_std6,misfit_cur_std7
    ...
    misfit_cur_mean1 = 0.0_CUSTOM_REAL
    ...
    nrec_process = nrec_proc 

    !---------------------band F1------------------! 
    do isrc=0,nsrc-1
    do ip=0,0
    write(filename,'(a,i6.6,a,i6.6,a)') trim(directory)//'/',isrc,'/proc',ip,'_deltat1_all.dat'
    OPEN (IIN,FILE= filename,STATUS='OLD',action='read',iostat=ier)
    ...
    ! take average of nsrc
    misfit_cur_mean1=misfit_cur_mean/num_mean

    !---------------misfit--------------!
    misfit_cur_mean_save = 0.0_CUSTOM_REAL
    ! source loop
    do isrc=0, nsrc-1
    print *,'source calculated',isrc
    do ip=0,0
    ! open file 
    write(filename,'(a,i6.6,a,i6.6,a)') trim(directory)//'/',isrc,'/proc',ip,'_misfit1.dat'
    OPEN (IIN,FILE= filename,STATUS='OLD',action='read',iostat=ier)
        read(IIN,*) temp
    close(IIN)
    !! sum over source (chi-squared)
    ...
    enddo !! ip
    enddo !! source loop
    misfit_cur_f1=misfit_cur_mean_save/num_mean
    print *,'nsrc=',nsrc
    print *,'num for f1 is:',num_mean,' misfit for f1 is:',misfit_cur_f1,'mean dt is:',misfit_cur_mean
    ...
    do ip=0,0
    ! open file 
    write(filename,'(a,i6.6,a,i6.6,a)') trim(directory)//'/',isrc,'/proc',ip,'_deltat1.dat'
    OPEN (IIN,FILE= filename,STATUS='OLD',action='read',iostat=ier)
    ...
    misfit_cur_std1=misfit_cur_std
   ...
   !! sum over source (chi-squared)
    ...
    if (num_filter == 2) then
            misfit_cur =  (misfit_cur_f1+misfit_cur_f2)/2
    else if (num_filter == 3) then
            misfit_cur =  (misfit_cur_f1+misfit_cur_f2+misfit_cur_f3)/3
    else if (num_filter == 4) then
            misfit_cur =  (misfit_cur_f1+misfit_cur_f2+misfit_cur_f3+misfit_cur_f4)/4
    else if (num_filter == 5) then
            misfit_cur =  (misfit_cur_f1+misfit_cur_f2+misfit_cur_f3+misfit_cur_f4+misfit_cur_f5)/num_filter
    else if (num_filter == 6) then
            misfit_cur =  (misfit_cur_f1+misfit_cur_f2+misfit_cur_f3+misfit_cur_f4+misfit_cur_f5+misfit_cur_f6)/num_filter
    else if (num_filter == 7) then
            misfit_cur =  (misfit_cur_f1+misfit_cur_f2+misfit_cur_f3+misfit_cur_f4+misfit_cur_f5+misfit_cur_f6+misfit_cur_f7)/num_filter
    else
            misfit_cur =   misfit_cur_f1
    endif
end subroutine sum_misfit_mean

subroutine sum_misfit_all(directory,misfit_cur_1,misfit_cur_2,misfit_cur_3,misfit_cur_4,misfit_cur_5,misfit_cur_6,misfit_cur_7,NPROC_DATA)
    use seismo_parameters
    implicit none
    integer :: ier,isrc,ip,NPROC_DATA
    real(kind=CUSTOM_REAL) ::temp,misfit_cur,misfit_cur_1,misfit_cur_2,misfit_cur_3,misfit_cur_4,misfit_cur_5,misfit_cur_6,misfit_cur_7,misfit_cur_average
    character(len=MAX_FILENAME_LEN) :: filename
    character(len=MAX_STRING_LEN) :: directory
    ...
    !! by chao
    if (num_filter == 2) then
            misfit_cur =  (misfit_cur_1+misfit_cur_2)/2
    else if (num_filter == 3) then
            misfit_cur =  (misfit_cur_1+misfit_cur_2+misfit_cur_3)/3
    else if (num_filter == 4) then
            misfit_cur =  (misfit_cur_1+misfit_cur_2+misfit_cur_3+misfit_cur_4)/4
    else if (num_filter == 5) then
            misfit_cur =  (misfit_cur_1+misfit_cur_2+misfit_cur_3+misfit_cur_4+misfit_cur_5)/num_filter
    else if (num_filter == 6) then
            misfit_cur =  (misfit_cur_1+misfit_cur_2+misfit_cur_3+misfit_cur_4+misfit_cur_5+misfit_cur_6)/num_filter
    else if (num_filter == 7) then
            misfit_cur =  (misfit_cur_1+misfit_cur_2+misfit_cur_3+misfit_cur_4+misfit_cur_5+misfit_cur_6+misfit_cur_7)/num_filter
    else
            misfit_cur =   misfit_cur_1
    endif
