...
subroutine initialize(directory,kernel_names,precond_name,model_names)
    use seismo_parameters
    implicit none
    integer :: ier,iker,imod,igllx,igllz,iglly,ispec
    ...
    !BH
    real(kind=CUSTOM_REAL), dimension(:,:,:,:),allocatable :: z_coordinate
    real(kind=CUSTOM_REAL) :: tempz,taperz
    ...
    !BH
    allocate(z_coordinate(NGLLX,NGLLY,NGLLZ,NSPEC))
    do myrank=0,nproc-1
        nspec_start=sum(nspec_proc(1:myrank))+1
        nspec_end=sum(nspec_proc(1:myrank))+nspec_proc(myrank+1)
        write(filename,'(a,i6.6,a)') &
            trim(directory)//'/misfit_kernel/proc',myrank,'_z.bin'
        print*,'LOAD z coordinate..... ',trim(filename)
        open(IIN,file=trim(filename),status='old',action='read',form='unformatted',iostat=ier)
        if (ier /= 0) then
            print*, 'Error: could not open coordinate file: ',trim(filename)
            stop 'Error: could not open coordinate file: '
        endif
        read(IIN) z_coordinate(:,:,:,nspec_start:nspec_end)
        close(IIN)
     enddo  ! myrank  
    if(Taper_surface)then
        print*,'+++++Taper between',z_bot,'to',z_top
        do iker=1,nker
            do igllx=1,NGLLX
                do iglly=1,NGLLY
                    do igllz=1,NGLLZ
                        do ispec=1,NSPEC
                            tempz=z_coordinate(igllx,iglly,igllz,ispec)
                            if(tempz>z_top) then
                                temp_store(igllx,iglly,igllz,ispec,iker) = 0.0
                            else if(tempz>z_bot) then
                                taperz=sin(3.1415926/2.0*(z_top-tempz)/(z_top-z_bot))
                                temp_store(igllx,iglly,igllz,ispec,iker) = temp_store(igllx,iglly,igllz,ispec,iker)*taperz**2
                            endif
                        enddo
                    enddo
                enddo
