program misfit_adjoint
    ...
    integer ::  ier,i,icomp,irec,send_data_tag,receive_data_tag,ifilter
    ...
    character(len=MAX_FILENAME_LEN) :: char_i
    ...
    if(nrec_proc > 0 ) then  ! non-zero file
        ...
        if(compute_adjoint .and. DISPLAY_TRACES) then
            ...
            !close(IOUT)
            do ifilter=1,num_filter
                write(char_i, '(I5)') ifilter        ! convert integer to char
                write(filename,'(a)') &
                    trim(input_dir)//'/DATA_obs/'//trim(adjustl(data_names(icomp)))//'_processed'//trim(adjustl(char_i))//'.bin'
                print*,'SAVE processed seismic_obs -- ',trim(filename)
                open(unit=IOUT,file=trim(filename),status='unknown',form='unformatted',iostat=ier)
                if (ier /= 0) then
                    print*, 'Error: could not open model file: ',trim(filename)
                ...
                if(ifilter==1)write(IOUT) seism_obs1(:,:)
                if(ifilter==2)write(IOUT) seism_obs2(:,:)
                if(ifilter==3)write(IOUT) seism_obs3(:,:)
                if(ifilter==4)write(IOUT) seism_obs4(:,:)
                if(ifilter==5)write(IOUT) seism_obs5(:,:)
                if(ifilter==6)write(IOUT) seism_obs6(:,:)
                if(ifilter==7)write(IOUT) seism_obs7(:,:)
                close(IOUT)

                write(filename,'(a)') &
                    trim(input_dir)//'/DATA_syn/'//trim(adjustl(data_names(icomp)))//'_processed'//trim(adjustl(char_i))//'.bin'
                print*,'SAVE processed seismic_syn -- ',trim(filename)
                open(unit=IOUT,file=trim(filename),status='unknown',form='unformatted',iostat=ier)
                if (ier /= 0) then
                    print*, 'Error: could not open model file: ',trim(filename)
                ...
                if(ifilter==1)write(IOUT) seism_syn1(:,:)
                if(ifilter==2)write(IOUT) seism_syn2(:,:)
                if(ifilter==3)write(IOUT) seism_syn3(:,:)
                if(ifilter==4)write(IOUT) seism_syn4(:,:)
                if(ifilter==5)write(IOUT) seism_syn5(:,:)
                if(ifilter==6)write(IOUT) seism_syn6(:,:)
                if(ifilter==7)write(IOUT) seism_syn7(:,:)
                close(IOUT)
            enddo
       ...
       ! step 3 -- misfit-adjoint
        ...
        A=max(num_AD1,1)+max(num_AD2,1)+max(num_AD3,1)+max(num_AD4,1)+max(num_AD5,1)+max(num_AD6,1)+max(num_AD7,1)    

        if(misfit_weighting) then
        A1= REAL(max(num_AD1,1)/A)
        ...
        misfit= 0.0_CUSTOM_REAL
        !misfit=  misfit1+misfit2+misfit3+misfit4+misfit5
        !BH
        if (num_filter == 2) then  
        print *,'num_filter=',num_filter
            misfit=  misfit1+misfit2
        else if (num_filter == 3) then
            misfit=  misfit1+misfit2+misfit3
        else if (num_filter == 4) then
            misfit=  misfit1+misfit2+misfit3+misfit4
        else if (num_filter == 5) then
            misfit=  misfit1+misfit2+misfit3+misfit4+misfit5
        else if (num_filter == 6) then
            misfit=  misfit1+misfit2+misfit3+misfit4+misfit5+misfit6
        else if (num_filter == 7) then
            misfit=  misfit1+misfit2+misfit3+misfit4+misfit5+misfit6+misfit7
        else                
            misfit=  misfit1
        endif
        ...
        if(compute_adjoint) then 
            ...
            if (num_filter == 2) then  
            print *,'num_filter=',num_filter
                seism_adj =  (seism_adj1+seism_adj2)/num_filter          
            else if (num_filter == 3) then
                seism_adj =  (seism_adj1+seism_adj2+seism_adj3)/num_filter
            else if (num_filter == 4) then
                seism_adj =(seism_adj1+seism_adj2+seism_adj3+seism_adj4)/num_filter
            else if (num_filter == 5) then
                seism_adj =(seism_adj1+seism_adj2+seism_adj3+seism_adj4+seism_adj5)/num_filter
            else if (num_filter == 6) then
                seism_adj =(seism_adj1+seism_adj2+seism_adj3+seism_adj4+seism_adj5+seism_adj6)/num_filter
            else if (num_filter == 7) then
                seism_adj =(seism_adj1+seism_adj2+seism_adj3+seism_adj4+seism_adj5+seism_adj6+seism_adj7)/num_filter
            else                
                seism_adj =   seism_adj1
            endif


            if (measurement_list == 'WD') then
              seism_adj =   seism_adj2
            endif
...
subroutine Absolute_diff(obs,syn,Tc,Te,Vp)
   ...
   ! get data 
    ...
    if (measurement_type=='WD') then
       misfit_all=misfit_all
       print *, "measurement_type::",measurement_type
    else
       print *, "measurement_type::",measurement_type
       if(Tc<=1) then
        !!  test the dis_sr(irec) should be > offset_far
        if(abs(misfit_value)>=Tc/2 .or. dis_sr(irec)<dismin .or. st_xval(irec)-x_source<0  ) then
        ...
        ! if(abs(misfit_value)>misfit_value_min .or. dis_sr(irec)<dismin .or. st_xval(irec)-x_source<0) then
        if(abs(misfit_value)>misfit_value_min .or. dis_sr(irec)<dismin) then
        !if(abs(misfit_value)>=misfit_value_min .or. dis_sr(irec)<50000  ) then
