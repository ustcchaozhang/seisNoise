...
if [ ! -z "$num_filter" ]; then
    sed -e "s#^INTEGER, PARAMETER :: num_filter=.*#INTEGER, PARAMETER :: num_filter=$num_filter #g"  $FILE > temp;  mv temp $FILE
fi


if [ ! -z "$misfit_value_min" ]; then
    sed -e "s#^REAL(KIND=8), PARAMETER :: misfit_value_min=.*#REAL(KIND=8), PARAMETER :: misfit_value_min=${misfit_value_min} #g"  $FILE > temp;  mv temp $FILE
fi
